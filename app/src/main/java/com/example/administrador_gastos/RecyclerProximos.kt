...
class RecyclerProximos : AppCompatActivity() {
    private lateinit var alarmManager: AlarmManager
    private lateinit var pendingIntent: PendingIntent
    private lateinit var miRecyclerView: RecyclerView
    private lateinit var miAdapter: AdaptadorProximos
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_recycler_proximos)
        ...
        val listaPagos = intent.getStringArrayListExtra("listaPagos")

        miRecyclerView = findViewById(R.id.recyclerViewProximos)
        ...
        miAdapter = AdaptadorProximos(
            listaPagos ?: ArrayList(),
            R.layout.recycler_view_item,
            object : AdaptadorProximos.OnItemClickListener {
                override fun onItemClick(name: String?, position: Int) {
                    val  alert = AlertDialog.Builder(this@RecyclerProximos)

                    alert.setMessage("¿Esta seguro de querer eliminar este pago?")
                    ...
                    alert.setNegativeButton("No"){_,_->
                        Toast.makeText(this@RecyclerProximos, "Operación abortada",Toast.LENGTH_LONG).show()
                    }
                    alert.setNeutralButton("Cancelar"){_,_->
                        Toast.makeText(this@RecyclerProximos, "Operación cancelada",Toast.LENGTH_LONG).show()
                    }
    ...
    private fun eliminarGasto(bd: DatabaseReference, gastoId: String, nombrePago : String, db : SQLiteDatabase) {
        ...
        bd.child(gastoId).removeValue().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                actualizarListaYAdaptador(nombrePago)
            } else {
    ...
    private fun actualizarListaYAdaptador(nombrePago: String) {
        // Primero, obtenemos la lista de gastos desde el adaptador
        val listaPagos = (miRecyclerView.adapter as AdaptadorProximos).dato

        // Filtramos el gasto a eliminar de la lista local
        val pagoCancelado = listaPagos.find { it.split(":")[0] == nombrePago }
        if (pagoCancelado != null) {
            // Eliminar el gasto de la lista
            listaPagos.remove(pagoCancelado)
            if(listaPagos.size == 0) finish()
