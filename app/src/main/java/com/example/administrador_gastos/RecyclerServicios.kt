...
class RecyclerServicios : AppCompatActivity() {
        ...
        miAdapter = AdaptadorServicios(
            ...
            object : AdaptadorServicios.OnItemClickListener {
                override fun onItemClick(name: String?, position: Int) {
                    val  alert = AlertDialog.Builder(this@RecyclerServicios)
                    ...
                    alert.setNegativeButton("No"){_,_->
                        Toast.makeText(this@RecyclerServicios, "Operación abortada",Toast.LENGTH_LONG).show()
                    }
                    alert.setNeutralButton("Cancelar"){_,_->
                        Toast.makeText(this@RecyclerServicios, "Operación cancelada",Toast.LENGTH_LONG).show()
                    }
    ...
    private fun actualizarListaYAdaptador(nombreServicio: String) {
        // Primero, obtenemos la lista de gastos desde el adaptador
        val listaGastos = (miRecyclerView.adapter as AdaptadorServicios).dato

        // Filtramos el gasto a eliminar de la lista local
        val gastoEliminado = listaGastos.find { it.split(":")[0] == nombreServicio }
